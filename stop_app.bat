...
REM 关闭PowerShell进程（如果是从PowerShell启动的）
for /f "tokens=2" %%i in ('tasklist /fi "imagename eq powershell.exe" ^| find "powershell.exe"') do (
    echo [关闭] 关闭PowerShell进程 %%i
    taskkill /pid %%i /f >nul 2>&1
)
